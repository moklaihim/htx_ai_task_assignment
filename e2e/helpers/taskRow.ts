 ...
 * spec that touches E2E-1 through E2E-6. Centralized here so each scenario
 * reads by *behavior* (find the row for this title, its assignee control,
 * its status control) rather than re-deriving selectors from the DOM
 * structure in every spec file.
 *
 * Every scenario gives its Task a title unique to the test run (a
 * timestamp/random suffix), so a plain substring match on the title cell is
 * enough to find the right row without colliding with other data sitting in
 * the shared compose stack's volume.
 */
